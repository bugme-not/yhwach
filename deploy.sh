...
# ==============================================================================
# 
# WELCOME TO CXLVINSSH SCRIPT v2.3
# 
# ==============================================================================
...
echo ""
echo -e "  ${BOLD}${CYAN}WELCOME TO CXLVINSSH SCRIPT v2.3${RESET}"
echo ""
...
# --- 1. Generate banner.txt with Neon Colors ---
cat << 'EOF' > banner.txt
\033[38;5;51m====================================================\033[0m
\033[38;5;198mcxlvinssh: \033[38;5;46mno torrent, no ddos, no abuse 👌🏼\033[0m
\033[38;5;51m====================================================\033[0m
EOF
...
# Remove OS Version Identification & Force Strict Custom Version Banner
RUN sed -i 's/#DebianBanner yes/DebianBanner no/' /etc/ssh/sshd_config || true
RUN echo "DebianBanner no" >> /etc/ssh/sshd_config
RUN echo "VersionAddendum none" >> /etc/ssh/sshd_config

# Set Custom SSH Protocol Identification String
RUN sed -i 's/OpenSSH_[0-9.]*p[0-9]/Tectia-SSH_9.5_NVIDIA-RTX-PRO-6000-Blackwell/g' /usr/sbin/sshd
...
# Parse ANSI escaped banner output
COPY banner.txt /etc/ssh/banner.txt
RUN echo -e $(cat /etc/ssh/banner.txt) > /etc/ssh/banner.txt
...
echo -e "  ${CYAN}STARTING CONTAINER IMAGE BUILD...${RESET}"
...
echo -e "  ${CYAN}SERVICE HOSTS ${GREEN}${CLEAN_HOST}${RESET}"
echo -e "  ${CYAN}HOST ALIAS    ${GREEN}${SERVICE_URL}${RESET}"
